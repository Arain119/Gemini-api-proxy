...
def render_dashboard_page():
    ...
    if usage_stats and models:
        selected_model = st.selectbox("选择模型", models, key="usage_model_select")
        ...
        if model_data:
            ...
            with st.expander("查看详细数据"):
                ...
                display_df['分钟使用率'] = display_df['分钟使用率'].fillna(0).apply(lambda x: f"{x:.1f}%")
                display_df['日使用率'] = display_df['日使用率'].fillna(0).apply(lambda x: f"{x:.1f}%")
...
def render_key_management_page():
    ...
    with tab1:
        if "cli_auth_session" not in st.session_state:
            st.session_state["cli_auth_session"] = None
        if "cli_auth_status" not in st.session_state:
...
def render_model_config_page():
    ...
    for model in models:
        ...
        with st.form(f"model_config_{model}"):
            alias_for = current_config.get('alias_for')
            if alias_for:
                st.caption(f"使用基础模型 `{alias_for}` 的限额配置")
            ...
            budget_col, include_col = st.columns(2)

            base_name = current_config.get('alias_for') or current_config.get('model_name') or model
            base_name = (base_name or "").split('/')[-1]
            if base_name.endswith('-search'):
                base_name = base_name[:-len('-search')]
            ...
            if 'flash' in base_lower and 'flash-lite' not in base_lower:
                budget_entries.append(("最大 (24k)", 24576))
            if 'pro' in base_lower:
                budget_entries.append(("最大 (32k)", 32768))

            budget_options = dict(budget_entries)
            model_budget = current_config.get('default_thinking_budget', -1)
            selected_budget_label = next((label for label, value in budget_options.items() if value == model_budget), "跟随全局")

            with budget_col:
                selected_budget = st.selectbox(
                    "默认思考预算",
                    options=list(budget_options.keys()),
                    index=list(budget_options.keys()).index(selected_budget_label),
                    help="设置该模型在未指定时的默认思考预算"
                )

            with include_col:
                include_default = bool(current_config.get('include_thoughts_default', 1))
                include_thoughts_default = st.checkbox(
                    "默认包含思考过程",
            ...
            if st.form_submit_button("保存配置", type="primary", width="stretch"):
                if not display_name or not display_name.strip():
                    ...
                    update_data = {
                        ...
                        "default_thinking_budget": budget_options[selected_budget],
                        "include_thoughts_default": include_thoughts_default
                    }
...
def render_system_settings_page():
    ...
    # 配置标签页
    tab_prompt, tab_stream, tab_balance, tab_failover, tab_cleanup, tab_experiment, tab_system = st.tabs([
        "提示词注入", "流式模式", "负载均衡", "故障转移", "自动清理", "实验性", "系统信息"
    ])
        ...
        if deepthink_data and deepthink_data.get('success'):
            ...
            with st.form("deepthink_form"):
                enabled = st.checkbox(
                    ...
                    help="开启后，包含 [deepthink] 关键词的请求将触发串行多轮推理流程"
                )
                st.caption("迭代轮数由模型实时决定，最多 7 轮。")
    ...
    with tab_system:
        ...
        version = status_data.get('version', '2.0')
