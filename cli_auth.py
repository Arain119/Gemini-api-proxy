...
class CliAuthManager:
    ...
    def _load_custom_client_config(self) -> Optional[Dict[str, Any]]:
        raw_json = os.getenv("GEMINI_CLI_CLIENT_CONFIG_JSON")
        if raw_json:
            try:
                return json.loads(raw_json)
            except json.JSONDecodeError as exc:
                logger.error("自定义 OAuth 配置 JSON 解析失败: %s", exc)
                raise HTTPException(status_code=500, detail="Invalid GEMINI_CLI_CLIENT_CONFIG_JSON value") from exc

        path = (os.getenv("GEMINI_CLI_CLIENT_CONFIG_PATH") or "").strip()
        if path:
            try:
                with open(path, "r", encoding="utf-8") as file:
                    return json.load(file)
            except FileNotFoundError as exc:
                logger.error("指定的 OAuth 配置文件不存在: %s", path)
                raise HTTPException(status_code=500, detail="GEMINI_CLI_CLIENT_CONFIG_PATH does not exist") from exc
            except json.JSONDecodeError as exc:
                logger.error("自定义 OAuth 配置文件解析失败: %s", exc)
                raise HTTPException(status_code=500, detail="Invalid OAuth client JSON file") from exc

        return None

    def _build_client_config(self, redirect_uri: str) -> Tuple[Dict[str, Any], bool]:
        custom_config = self._load_custom_client_config()
        using_default_credentials = False

        if custom_config:
            config = json.loads(json.dumps(custom_config))  # 深拷贝，避免修改原始配置
            web_config = config.get("web")
            if not web_config and "installed" in config:
                web_config = config["installed"]
                config = {"web": dict(web_config)}
            elif web_config:
                config["web"] = dict(web_config)

            if not config.get("web"):
                raise HTTPException(status_code=500, detail="OAuth client config must contain a 'web' section")
        ...
        redirect_list = list(web_section.get("redirect_uris") or [])
        if redirect_uri not in redirect_list:
            redirect_list.append(redirect_uri)
        web_section["redirect_uris"] = redirect_list

        return config, using_default_credentials
    ...
    def _start_remote_authorization(self) -> Dict[str, Any]:
        ...
        client_config, using_default_credentials = self._build_client_config(callback_url)

        if using_default_credentials:
            raise HTTPException(
                status_code=400,
                detail=(
                    "远程回调模式需要自定义的 Google OAuth 客户端，请在环境变量中配置 "
                    "GEMINI_CLI_CLIENT_ID/GEMINI_CLI_CLIENT_SECRET，或提供 "
                    "GEMINI_CLI_CLIENT_CONFIG_JSON/GEMINI_CLI_CLIENT_CONFIG_PATH。"
                ),
            )
    ...
    def _start_loopback_authorization(self) -> Dict[str, Any]:
        ...
        client_config, _ = self._build_client_config(redirect_uri)
        flow = Flow.from_client_config(
            client_config,
            scopes=DEFAULT_SCOPES,
