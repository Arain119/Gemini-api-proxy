...
class CliAuthManager:
    ...
    def __init__(self, *, database_factory: Optional[Any] = None) -> None:
        ...
        self._raw_client_id = (os.getenv("GEMINI_CLI_CLIENT_ID") or "").strip()
        self._raw_client_secret = (os.getenv("GEMINI_CLI_CLIENT_SECRET") or "").strip()
        self._client_id, self._client_secret = self._resolve_client_credentials()
        self._custom_client_configured = self._determine_custom_client()
        self._missing_client_fields = self._determine_missing_client_fields()
    ...
    def _resolve_client_credentials(self) -> Tuple[str, str]:
        client_id = self._raw_client_id or CLI_DEFAULT_CLIENT_ID
        client_secret = self._raw_client_secret or CLI_DEFAULT_CLIENT_SECRET
        return client_id, client_secret

    def _determine_custom_client(self) -> bool:
        if not self._raw_client_id or not self._raw_client_secret:
            return False
        if (
            self._raw_client_id == CLI_DEFAULT_CLIENT_ID
            and self._raw_client_secret == CLI_DEFAULT_CLIENT_SECRET
        ):
            return False
        return True

    def _determine_missing_client_fields(self) -> Tuple[str, ...]:
        missing: list[str] = []
        if not self._raw_client_id:
            missing.append("GEMINI_CLI_CLIENT_ID")
        if not self._raw_client_secret:
            missing.append("GEMINI_CLI_CLIENT_SECRET")
    ...
    def _start_remote_authorization(self) -> Dict[str, Any]:
        ...
        if self._missing_client_fields:
            missing_list = "、".join(self._missing_client_fields)
            raise HTTPException(
                status_code=400,
                detail=(
                    "检测到云端回调模式，但缺少以下 OAuth 环境变量："
                    f"{missing_list}。请在部署环境中配置 Google OAuth Web 客户端的 ID 和密钥。"
                ),
            )

        if not self._custom_client_configured:
            raise HTTPException(
                status_code=400,
                detail=(
                    "当前部署启用了云端回调，但仍在使用 Gemini CLI 默认的 OAuth 客户端。\n"
                    "请在 Google Cloud Console 创建一个“Web 应用”类型的 OAuth 客户端，"
                    "将以下地址加入授权重定向 URI：\n"
                    f"{callback_url}\n"
                    "然后把客户端 ID/密钥分别写入 GEMINI_CLI_CLIENT_ID 和 GEMINI_CLI_CLIENT_SECRET 环境变量后重试。"
                ),
            )
